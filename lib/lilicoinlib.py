...
def is_valid_lilicoin_address(address, network='mainnet'):
    ...
    lilicoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid lili address
        return False

    if (address_version != lilicoin_version):
        return False
...
# shims 'til we can fix the lilicoind side
def SHIM_serialise_for_lilicoind(sentinel_hex):
    ...
    # superblock => "trigger" in lilicoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # lilicoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    lilicoind_hex = serialise(obj)
    return lilicoind_hex


# shims 'til we can fix the lilicoind side
def SHIM_deserialise_from_lilicoind(lilicoind_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(lilicoind_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('lilicoin.conf')
